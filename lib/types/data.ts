...
export interface Customer {
  ...
  holdout_group: boolean; // For RCCR metric comparison
}
...
export interface ProductEvidence {
  ...
  category_name: string;
  brand_name: string;
  reorder_rate: number; // e.g. 0.89 -> 89%
  return_rate: number; // e.g. 0.012 -> 1.2%
  seller_consistency_score: number; // e.g. 98.6 -> 98.6%
  seller_name: string;
