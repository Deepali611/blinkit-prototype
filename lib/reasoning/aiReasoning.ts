...
/**
 * Deterministic Fallback Engine for Stage 2 Reasoning
 * Used when GROQ_API_KEY is not set or network request fails.
 * Guarantees zero downtime and 100% path safety compliance.
 */
export function generateFallbackReasoning(input: AIReasoningInput): AIReasoningOutput {
  ...
  if (path_type === "PATH_A_KNOWN_SIGNAL") {
    ...
    return {
      ...
      path_b_safety_compliant: true, // Path A is allowed to reference confirmed history
      generated_by: "DETERMINISTIC_FALLBACK",
    };
  }

  // Path B: Inferred Signal (No explicit negative signal on record)
  // Must infer most probable failure type without claiming specific unconfirmed incident occurred!
  ...
  return {
    ...
    reassurance_body: body,
    recommended_action: action,
    path_b_safety_compliant: true, // Ensured general tone without personal claims
    generated_by: "DETERMINISTIC_FALLBACK",
  };
}

/**
 * Server-side Groq LLM integration for Stage 2 Reasoning
 */
export async function performAIReasoning(input: AIReasoningInput): Promise<AIReasoningOutput> {
  const apiKey = process.env.GROQ_API_KEY;

  if (!apiKey) {
    console.log("GROQ_API_KEY not found. Using high-quality deterministic fallback engine.");
...
`;
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`Groq API returned HTTP ${response.status}. Falling back.`);
      return generateFallbackReasoning(input);
    }
